...
- Role list: `src/models/roles.js`, mirrored by `functions/roles.js`; `firestore.rules` `isBaseLevel()` lists the base roles. Keep all three in sync.
- Three dashboards, by Role: Admin and Department Head (both inside `DashboardShell`), and `MemberView` for every base-level role.
