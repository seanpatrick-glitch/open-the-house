...
export default function Volunteers({ show }) {
            ...
            {volunteers.map((v, i) => (
              <tr key={i} className="group">
                <td className="py-2 pl-2 pr-3">
                  <input type="text" value={v.shift || ''} onChange={(e) => updateRow(i, 'shift', e.target.value)}
                ...
                <td className="py-2 px-3">
                  <input type="number" min="0" value={v.needed ?? 5} onChange={(e) => updateRow(i, 'needed', parseInt(e.target.value) || 0)}
                    className="w-full border border-gray-200 rounded-lg px-2 py-2 text-sm text-center focus:outline-none focus:ring-1 focus:ring-amber-400" />
                </td>
                <td className="py-2 px-3">
                  <input type="number" min="0" value={v.filled ?? 0} onChange={(e) => updateRow(i, 'filled', parseInt(e.target.value) || 0)}
                    className={`w-full border rounded-lg px-2 py-2 text-sm text-center focus:outline-none focus:ring-1 focus:ring-amber-400 ${
                      (v.filled || 0) >= (v.needed || 5) ? 'border-green-300 bg-green-50' : 'border-gray-200'
                    }`} />
                </td>
                <td className="py-2 px-3">
                  <input type="text" value={v.notes || ''} onChange={(e) => updateRow(i, 'notes', e.target.value)}
                    placeholder="Notes"
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-amber-400" />
                </td>
