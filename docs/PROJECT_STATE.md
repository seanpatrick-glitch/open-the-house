...
**Next build priority:**
- Combined App Architecture briefing written and ready (July 15 Feature Roadmap continuation), covering four new areas: thread-based messaging (broadcast fan-out, person-initiated threads, alert-only email notifications with deep link, two-way replies, read receipts via platform open), department timelines (separate task spaces rolling up to org view, collapsed department rows, admin-only promotion toggle), task handoff (acceptance flow, handoff subcollection, currentAssigneeUid and handoffPending on task document, task history log subcollection), and multi-person task assignment (primaryAssigneeUid and contributorUids array, primary or admin completes only). Second queued App Architecture briefing — see Section 7.
...
- Two App Architecture briefings were queued as of the July 15 Feature Roadmap session — person access model/check-in (built July 19 directly by Code Interpreter without a dedicated App Architecture session, see Section 6) and messaging/department timelines/task handoff/multi-person assignment (see Section 2, still pending). Originally recommended to address both together in one combined App Architecture session; only the second briefing now remains queued for that session.
