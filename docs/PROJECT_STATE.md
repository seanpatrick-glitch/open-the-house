...
- Combined App Architecture briefing written and ready to paste (July 15 Feature Roadmap session), covering three areas: person class access model (status enum plus staff boolean), email invite to an existing person record (accountUid linking pattern), and check-in Phase 1 (presence-only, assignment-attached records, QR token time-validity window). Needs an App Architecture session to lock data model and security rules — including the still-open staff toggle security rule mechanism (custom claim vs rule-time check) — before Code Interpreter builds any of it.
...
- Staff toggle security rule mechanism undecided: custom claim vs rule-time check. Specced July 15 in Feature Roadmap session that the staff toggle is a permission layer on Active persons, not a separate role — enforcement mechanism itself is an open item for App Architecture.
